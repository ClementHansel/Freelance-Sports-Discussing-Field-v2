...
export const useMostViewedTopics = (page = 1, limit = 10) => {
  ...
  return useQuery<PaginatedMostViewedTopicsResult>({
    ...
    queryFn: async () => {
      ...
      // Directly cast to MostViewedTopic[] or unknown as MostViewedTopic[]
      // Then map to ensure all properties are present and correctly typed.
      const topics = (topicsResult.data as unknown as MostViewedTopic[]).map(
        (item) => ({
          ...item,
      ...
      ); // No need for final 'as MostViewedTopic[]' if map correctly returns it
