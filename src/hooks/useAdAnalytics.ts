...
export const useAdAnalytics = (
  ...
  const { data: adSpaceData, isLoading: adSpaceLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const grouped =
        data?.reduce((acc: Record<string, AdAnalyticsData>, item) => {
          const spaceId = item.ad_space_id;
          if (!acc[spaceId]) {
            acc[spaceId] = {
              ad_space_id: spaceId,
              ad_space_name:
                (item.ad_spaces as { name: string | null })?.name || "Unknown", // Ensure name is handled as string | null
              impressions: 0,
              clicks: 0,
              revenue: 0,
              ctr: 0,
          ...
          acc[spaceId].impressions += item.impressions ?? 0; // Use nullish coalescing
          acc[spaceId].clicks += item.clicks ?? 0; // Use nullish coalescing
          acc[spaceId].revenue += item.revenue ?? 0; // Use nullish coalescing
